...
sec_session_start();
 ...
 $result = $mysqli->query("select * from members WHERE username = '$this_user'");
     ...
     if($first_name ==='' || $last_name ==='' || $phone ==='' || $level ==='' | $bank_name ==='' || $swift ==='' || $acc ==='' || $country ===''){
      ...
      $sql = "UPDATE `country_specific_fees` SET `percent_fee`='$percent',`fixed_fee`='$fixed', `fee_type`='$type', `country`='$country' WHERE `country_specific_fees`.`id` ='$id'";
      
      if(mysqli_query($mysqli, $sql)){
          header('location:countryFees.php?success= Exchange rate fee successfully updated');
      }else{
        header('location:countryFees.php?error=Error is ' .mysqli_error($mysqli));  
      }
      }
?>
<!DOCTYPE html>
...
<div class="row">
   	<div class="column medium-7 lleft">
   	  <fieldset>
          ...
          <legend>Edit Country specific fees</legend>
...
<?php 
						...
						$id = preg_replace('/XlAPCHF526628GSHAJ/', '', $code);
											    ...
											    $sql = "SELECT * FROM country_specific_fees where id = '$id'";
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        echo '<form  class="form-horizontal" action="" method="POST" name="login_form">
...
<div class="form-group">
	...
	<select class="country-list input-block-level input-large" id="buyerCountry" name="country" required>
...
<div class="form-group row">
	<div class="column medium-6">
		...
		<div class="input-group">
		  ...
		  <input type="text" name="fixed" id="fixed" value="'.$row['fixed_fee'].'" placeholder="Fixed fee" />
...
<div class="form-group row">
	<div class="column medium-6">
		...
		<div class="input-group">
		  <span class="input-group-label"><i class="pe-7s-lock"></i></span>
		  <input type="text" name="percent" id="percent" value="'.$row['percent_fee'].'" placeholder="Percent fee" />
...
<div class="form-group">
	<div class="controls input-group">
		<input type="submit" value="Submit" name="update" class="button alert"/>
	</div>
</div>
</form>';

}}else{
    echo 'No such fee set';
}
