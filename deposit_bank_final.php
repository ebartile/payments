...
<div class="validation-summary-valid" data-valmsg-summary="true"><ul><li style="display:none"></li>
				...
				$amount = $_GET['amount'];
					...
					$sql = "SELECT * FROM banks where value ='$bank'";
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        echo'
        <div>Deposit money via the following details, enter your user name or customer id as deposit reference number when paying and submit.</div>
    <hr>
    <div>Business name: '.$row['business_name'].'</div>
        <div>Bank name: '.$row['bank_name'].'</div>
         <div>Account number: '.$row['account_number'].'</div>
          <div>Swift Code: '.$row['swift_code'].'</div>
           <div>Amount to pay: '.$member_currency.'. '.$amount.'</div>
           <hr>
           <form  class="form-horizontal" action="php/process_bank_transactions.php" method="POST" name="login_form">
...
<div class="form-group row">
	<div class="column medium-6">
		<label for="buyerPassword" class="control-label">Transaction code <span class="field-validation-valid" data-valmsg-for="password" data-valmsg-replace="false">*</span></label>
		<div class="input-group">
		  <span class="input-group-label"><i class="pe-7s-lock"></i></span>
		 <input type="text" name="refnum" id="refnum" value="" placeholder="Transaction Reference number" />
...
<div class="form-group">
	<div class="controls input-group">
		<input type="submit" value="submit" name="submit" class="button alert"/>
	</div>
</div>
</form>
       ...
       echo 'No such a Bank supported by Payments';
    }
