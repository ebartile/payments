...
<head>
        ...
        <title>Request money - Payment Gateway</title>
...
<div class="row">
   	<div class="column medium-7 lleft">
   	  <fieldset>
          ...
          <legend>Confirm to Request money</legend>
        	<?php
					$email = $_GET['email'];
...
if ($resultuser->num_rows > 0) {
    ...
    echo '<span style="color:green;"> You are about to Request a payment of '.$member_currency.' ' . $amount1. ' from ' . $first_name . ' ' .$last_name. '</span>';
    
}else{
    $email = $_GET['email'];
    $amount1 = $_GET['amount'];
    $amount = round($amount1/$member_rate, 2);
    echo '<span style="color:orange;">You are about to request a payment of '.$member_currency.' ' . $amount1. ' from an unregistered person on '. $email . ' . He/she will see this request as soon as he/she registers on Payments</span>';
}
...
<form  class="form-horizontal" action="php/process_request.php" method="POST" name="login_form">



<div class="form-group">
	<label class="control-label" for="buyerEmail">Email of the payer<span class="field-validation-valid" data-valmsg-for="email" data-valmsg-replace="false">*</span></label>
...
<div class="form-group">
	<div class="controls input-group">
		<input type="submit" value="Proceed" name="request" class="button alert"/>
	</div>
</div>
</form>
<?php
