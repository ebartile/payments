...
<head>
        ...
        <title>Withdraw - Payment Gateway</title>
...
<form  class="form-horizontal" action="php/process_withdrawals.php" method="POST" name="login_form">



<div class="form-group">
	<label class="control-label" for="buyerEmail">You preferred withdrawal method<span class="field-validation-valid" data-valmsg-for="email" data-valmsg-replace="false">*</span></label>
	<div class="controls input-group">
		<span class="input-group-label"><i class="pe-7s-mail"></i></span>
		<select name="method_name" id="method_name" required>
													<option value="" selected hidden>Select a Withdrawal method</option>
																	<?php
        
        $result = $mysqli->query("select method_name, value from payment_methods WHERE allowed = 'yes' AND withdrawal_method = 'yes' AND allowed_country ='$country' || allowed_country = 'ALL'");
        while ($row = $result->fetch_assoc()) {
            $method_name = $row['method_name'];
            $value = $row['value'];
             echo '<option value="'.$value.'">'.$method_name.'</option>';
        }
        ?>
...
<div class="form-group">
	<div class="controls input-group">
		<input type="submit" value="Proceed to withdraw" name="withdraw" class="button alert"/>
	</div>
</div>
</form>
<?php
