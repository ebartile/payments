...
<form  class="form-horizontal" action="php/process_deposit.php" method="POST" name="login_form">



<div class="form-group">
	<label class="control-label" for="buyerEmail">Deposit method<span class="field-validation-valid" data-valmsg-for="email" data-valmsg-replace="false">*</span></label>
	<div class="controls input-group">
		<span class="input-group-label"><i class="pe-7s-mail"></i></span>
		<select name="deposit_method" id="deposit_method">
													<option value="" selected hidden>Select a Deposit method</option>
																	<?php
        
        $result = $mysqli->query("select method_name, value from payment_methods WHERE allowed = 'yes' AND deposit_method = 'yes' AND allowed_country ='$country' || allowed_country = 'ALL'");
        while ($row = $result->fetch_assoc()) {
