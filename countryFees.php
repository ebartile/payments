...
<head>
        ...
        <title>Country Gateway fees</title>
...
<div class="row">
   	<div class="column medium-7 lleft">
   	  <fieldset>
          <legend>Fees per country</br>
					</legend>
					<p style="color:red;"> Note: If a user's set country is not set here, the default gateway fees apply. <a href="addFees.php"><br><button class="button primary">Add new country with fees</button></a></p>
         <p style="font-size:11px;">Note: The total fees applied will be %age fee + Fixed fee</p>
...
$sqluser = "SELECT * FROM country_specific_fees ORDER BY country ASC";
    ...
    while($row = $resultuser->fetch_assoc()) {
        ...
        echo
											'<tr>
													...
													<td>'.$row['fee_type'].'</td>
													<td>'.$row['fixed_fee'].'</td>
													<td>'.$row['percent_fee'].'</td>
													
														<td><button class="button alt small fit"><a href="edit_countryfees.php?code=XlAPCHF526628GSHAJ'.$row['id'].'">Edit</a></button></td>
												</tr>';
												
    }}else{
        echo 'No country specific fees set';
    }
