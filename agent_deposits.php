...
<head>
        ...
        <title>My agency deposits  - Payment Gateway</title>
...
<div class="row">
   	<div class="column medium-7 lleft">
   	  <fieldset>
          <legend>Deposits at my agent</legend>
...
if ($resultuser->num_rows > 0) {
    ...
    if($level !=='admin'){
											    ...
											    $sql = "SELECT * FROM transactons where sender_name = '$id' AND transaction_type ='Deposit' ORDER BY date DESC LIMIT 10";
$result = $mysqli->query($sql);
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        $status = $row['status'];
        if($status =='Pending'){
            $action = '<form action="php/process_agentDeposits.php" method="POST" name="login_form">
        ...
        echo
											'<tr>
													...
													<td><span style="color:#2B547E;">' .$row['naration']. '</span> '.$row['sender_name'].' '.' </br> '.$row['status'].' '.$action.'</td>
