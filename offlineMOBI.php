...
<head>
        ...
        <title>Deposit via  Mobile Wallet Gateways</title>
...
<div class="row">
   	<div class="column medium-7 lleft">
   	  <fieldset>
          ...
          <legend>Supported Mobile Wallets Methods</legend>
...
<form  class="form-horizontal" action="deposit_MOBI_final.php?MOBI=$MOBI_name&amount=$amount" method="GET" name="login_form">



<div class="form-group">
	<label class="control-label" for="buyerEmail">Select a Mobi payment method<span class="field-validation-valid" data-valmsg-for="email" data-valmsg-replace="false">*</span></label>
	<div class="controls input-group">
		<span class="input-group-label"><i class="pe-7s-mail"></i></span>
			<select name="MOBI" id="bank_name" required>
													<option value="" selected hidden>Select a Mobile Gateway </option>
												
												<?php
        
        $result = $mysqli->query("select MOBI_name, value from offline_MOBI WHERE allowed = 'yes' AND allowed_country ='$country'");
        while ($row = $result->fetch_assoc()) {
...
<div class="form-group">
	<div class="controls input-group">
		<input type="submit" value="Deposit" name="deposit" class="button alert"/>
	</div>
</div>
</form>
<?php
