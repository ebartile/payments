...
<head>
        ...
        <title>Pending Deposits  - Payment Gateway</title>
...
<div class="row">
   	<div class="column medium-7 lleft">
   	  <fieldset>
          <legend>Pending Deposits  - Payment Gateway</legend>
...
if ($resultuser->num_rows > 0) {
    ...
    if($level !=='admin'){
											    ...
											    $sql = "SELECT * FROM transactons where sender_name = '".$_SESSION['username']."' OR recipient_name = '".$_SESSION['username']."' AND status ='Pending' AND transaction_type ='Deposit' ORDER BY date DESC LIMIT 5";
$result = $mysqli->query($sql);
...
if ($result->num_rows > 0) {
    ...
    echo "<p style='color:red;text-align:center;'>No Pending deposits waiting for approval. Only Manual deposits will appear here first for admin to approve.</p>";
}		
	}else{
	        $sql = "SELECT * FROM transactons WHERE status ='Pending' AND transaction_type ='Deposit' ORDER BY transaction_id DESC";
$result = $mysqli->query($sql);
    ...
    while($row = $result->fetch_assoc()) {
        ...
        echo
											'<tr>
													...
													<form action="php/admin_processdeposits.php" method="POST" name="login_form">
