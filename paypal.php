   	...
   	<div class="column medium-7 lleft">
   	  <fieldset>
          ...
          <legend>
					<?php
					$amount = $_GET['amount'];
    echo '<span style="color:green;"> You are about to deposit ' . $amount. ' USD via Paypal.</span>';
...
?></legend>
	...
	<hr>   
<?php
$sqlpaypal = "SELECT * FROM site_config";
$resultpaypal = $mysqli->query($sqlpaypal);
if ($resultpaypal->num_rows > 0) {
    // output data of each row
    while($row = $resultpaypal->fetch_assoc()){
        $amount = $_GET['amount'];
        $currency = $row['gateway_currency'];
	$paypal_mode = $row['paypal_gateway_mode'];
	$site_name = $row['site_name'];
    $paypal_cencel_url = $row['paypal_cencel_url'];
     $paypal_confirmation_url = $row['paypal_confirm_url'];
...
elseif($paypal_mode ==='live'){
    ...
    header('location:paypal.php?error=error10');
}	
	echo '<form action="'.$paypal_link.'" method="POST" name="login_form">
        ...
        <input type="hidden" name="cancel_return" value="'.$paypal_cencel_url.'"> 

<input type="hidden" name="return" value="'.$paypal_confirmation_url.'"> 
...
<div style="">
        ...
        src="images/paypal_checkout.png" alt="PayPal - The safer, easier way to pay online"> 
