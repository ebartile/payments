<?php
 ...
 $sqlex_fees = "SELECT * FROM `exchange_rate_fees` where id = 1";
$resultex_fees = $mysqli->query($sqlex_fees);

if ($resultex_fees->num_rows > 0) {
    while($row = $resultex_fees->fetch_assoc()) {
		$fixed = $row['fixed'];
		$percent = $row['percent'];
    } }
 //get base currency
...
foreach ($exchangeRates as $obj) {
    ...
    $value1 = $value- ($value * $percent/100 + $fixed);
    //UPDATE into the exchange rates rates table
    
    $SQL = "UPDATE `currency` SET `e_rate`='$value1' WHERE `currency`.`code`='$key'";
