...
sec_session_start();
  ...
  if(isset($_POST['update'])){
      ...
      $sql = "UPDATE `default_gateway_fees` SET `fee_amt`='$fee_amt',`type`='$type',`for_type`='$for_type' WHERE `default_gateway_fees`.`id` ='$id'";
      
      if(mysqli_query($mysqli, $sql)){
          header('location:gatewayFees.php?success='.$for_type. ' methods default fee successfully updated');
      }else{
        header('location:gatewayFees.php?error=Error is ' .mysqli_error($mysqli));  
      }
...
<div class="row">
   	<div class="column medium-7 lleft">
   	  <fieldset>
          <legend>Edit gateway fee</legend>
...
<table>
						...
						$id = preg_replace('/XlAPCHF526628GSHAJ/', '', $code);
											    ...
											    $sql = "SELECT * FROM default_gateway_fees where id = '$id'";
$result = $mysqli->query($sql);
    ...
    while($row = $result->fetch_assoc()) {
        ...
        echo
											'<form action="" method="POST"><tr>
													<td>For?: '.$row['for_type'] . '</td>
												</tr>
												
										<tr><td>Type (fixed or percent): <span style="color:#2B547E;"><input type="text" value="' .$row['type']. '"placeholder="Type can be Fixed or percent" name="type" required></span></td> </tr>		
									<tr>	<td>Fee amount:		
													<input type="text" value="' .$row['fee_amt']. '"placeholder="enter fee amount" name="fee_amt" required>
													
										<input type="hidden" value="'.$row['for_type'].'" name="for_type">			
													
												<input type="hidden" value="'.$row['id'].'" name="id">		
													</td></tr>			
												
										<tr>	<td>		
													<input class="button alert" type="submit" value="Update" name="update" ></td></tr>	
