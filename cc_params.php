...
<div class="row">
   	<div class="column medium-7 lleft">
   	  <fieldset>
          <legend>Managed Credit card gateways settings - Payment Gateway</legend>
 ...
 <!---Self managed ------->
					    <h3>Self managed card params</h3>
					    <hr>
					    <table>
											<thead>
											<tr>
											    <th>Allow saving of card details<br> <abbr style="color:red;" title=" Set to YES only when you have the necessary requirements for keeping cards save. Right from your server, to web forms to Database, to office, to competent personnel and security surveilance">Info</abbr></th>
											    <th>Allow emailing of card details<br>
											   <abbr style="color:red;" title=" Set to YES only if you are ready to delete card data after using it and you can keep your emails secure from intruders. ">Info</abbr> 
...
$sql = "SELECT * FROM db_card_settings";
$result = $mysqli->query($sql);
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        echo
											'		<tr>
													<td>'.$row['to_be_saved'].'</td>
													
												
												<td> '.$row['to_be_emailed'] . '</td>	
											
								
												
												<td> <a href="edit_selfcc.php?gateway='.$row['id'].'">Edit</a></td>
												</tr>
    ...
    echo "<tr><td><p style='color:red;text-align:center;'>No Installed Managed Credit card gateways</p></td><td>";
					...
					?>
									...
									<!---- fully managed-->
									<h3>Fully managed credit card gateway params</h3>
					   <hr>
				<table>
											<thead>
											<tr>
...
$sql = "SELECT * FROM credit_card_params";
$result = $mysqli->query($sql);
    ...
    while($row = $result->fetch_assoc()) {
        ...
        echo
											'		<tr>
												...
												<td> '.$row['mode'] . '</td>	
												<td> <a href="edit_cc.php?gateway='.$row['id'].'">Edit</a></td>
												</tr>
