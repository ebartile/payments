...
sec_session_start();
  ...
  if(isset($_POST['update'])){
      ...
      $sql = "UPDATE `exchange_rate_fees` SET `percent`='$percent',`fixed`='$fixed' WHERE `exchange_rate_fees`.`id` ='$id'";
      
      if(mysqli_query($mysqli, $sql)){
          header('location:exchangeRate_fees.php?success= Exchange rate fee successfully updated');
      }else{
...
<table>
						...
						$id = preg_replace('/XlAPCHF526628GSHAJ/', '', $code);
											    ...
											    $sql = "SELECT * FROM exchange_rate_fees where id = '$id'";
$result = $mysqli->query($sql);
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        echo
										...
										<tr><td>Fixed: <span style="color:#2B547E;"><input type="text" value="' .$row['fixed']. '"placeholder="Input the fixed rate" name="fixed" required></span></td> </tr>		
									<tr>	<td>Percent:		
													<input type="text" value="' .$row['percent']. '"placeholder="enter percent fee" name="percent" required>
